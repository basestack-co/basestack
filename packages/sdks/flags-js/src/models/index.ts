...
export class FlagsSDK {
  ...
  constructor(config: SDKConfig) {
    ...
    this.cache = new Map();
    this.fetchImplementation = this.config.fetchImpl;
  ...
  private async request<T>(
    ...
    // Parse dates and cache the result
  ...
  /**
   * Parse and transform date fields in flag data
   * @param data - Raw flag data
   * @returns Parsed flag data with Date objects
   */
  private parseData(data: any): Flag | { flags: Flag[] } {
    if ("flags" in data) {
      return {
        flags: data.flags.map((flag: Flag) => ({
          ...
          expiredAt: flag.expiredAt ? new Date(flag.expiredAt) : null,
        })),
      };
    }

  ...
  .post(
    ...
    serve<CheckDataForSpamPayload>(
      async (context) => {
        ...
        console.info(
          `Job: Basestack Forms - Check Spam - Preparing to check data for spam: ${JSON.stringify(data)} with submission ID: ${submissionId}`
        );
  ...
  .post(
    ...
    serve<SendDataToExternalWebhookPayload>(
      async (context) => {
        ...
        console.info(
          `Job: Basestack Forms - External Webhook - Preparing to send data to external webhook: ${url}`
        );
        ...
        await context.run("send-data-to-external-webhook-step", async () => {
          ...
          console.info(
            `Job: Basestack Forms - External Webhook - Data sent to the external webhook with status: ${res.status}`
          );
        });

        console.info(
          "Job: Basestack Forms - External Webhook - ✨ Data successfully sent to the external webhook ✨"
        );
      },
      {
        receiver: new Receiver({
        ...
        failureFunction: async ({
          ...
          console.error(
            `Job: Basestack Forms - External Webhook - status = ${JSON.stringify(failStatus)} response = ${JSON.stringify(failResponse)} headers = ${JSON.stringify(failHeaders)} context = ${JSON.stringify(context)} `
          );
        },
      }
    )

...
We use [Prisma ORM ↗](https://www.prisma.io/?ref=basestack.co) in our project, which requires a direct connection to the database for migrations. Supabase supports both pooled and direct connections to the same database, configurable via Environment Variables for deploying our platform. Follow this official guide on integrating [Supabase with Prisma ↗](https://supabase.com/partners/integrations/prisma). Obtaining your Postgres connection string from your Supabase dashboard should be a straightforward process. The guide emphasizes the importance of using a direct connection for migrations.

...
| ----------------- | --------------------------------------------------------------------------- | -------- | ------------------------------------------------- |
| `AUTH_SECRET` | Used to encrypt the NextAuth.js JWT, and to hash email verification tokens. | true     | [generate](https://generate-secret.vercel.app/32) |
...
<Tabs items={["Feature Flags", "Forms"]}>
  ...
  <Tabs.Tab>
    ...
    Because the Forms Platform uses background jobs to handle tasks such as sending emails, you need to configure the following environment variables:
      ...
      <Callout type="info" emoji="ℹ️">
          Why [UpStash ↗](https://upstash.com/)? Since the Basestack Platform can

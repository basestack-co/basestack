...
<Cards>
  <Cards.Card title="Neon - Serverless Postgres" href="/self-hosting/database/neon" />
  <Cards.Card title="Supabase Postgres" href="/self-hosting/database/supabase" />
</Cards>
...
For the `AUTH_SECRET` variable you can use `openssl rand -base64 32` or generate a [random value](https://generate-secret.vercel.app/32).

| Property          | Description                                                                 | Required | Example                                           |
| ----------------- | --------------------------------------------------------------------------- | -------- | ------------------------------------------------- |
| `AUTH_SECRET` | Used to encrypt the NextAuth.js JWT, and to hash email verification tokens. | true     | [generate](https://generate-secret.vercel.app/32?ref=basestack.co) |
| `AUTH_URL`    | Set to the canonical URL of your site                                       | true     | https://your-basestack-instance.com               |
...
<Callout type="info" emoji="ℹ️">
  ...
  ↗](https://sendgrid.com/en-us?ref=basestack.co), [Mailgun ↗](https://www.mailgun.com/?ref=basestack.co), or
  [Resend ↗](https://resend.com?ref=basestack.co) to send emails.
</Callout>
...
<Tabs items={["Feature Flags", "Forms"]}>
  ...
  <Tabs.Tab>
    ...
    Because the Forms Platform uses background jobs to handle tasks such as sending emails, you need to configure the following environment variables:
      ...
      <Callout type="info" emoji="ℹ️">
          Why [UpStash ↗](https://upstash.com/?ref=basestack.co)? Since the Basestack Platform can
          run in serverless environments, using a service like UpStash QStash to handle
          background jobs is essential. QStash is a service designed to handle task scheduling and message queuing in
          a serverless
          environment. It’s particularly useful for serverless or edge applications that need to offload tasks
          asynchronously or schedule them for future execution.
      </Callout>

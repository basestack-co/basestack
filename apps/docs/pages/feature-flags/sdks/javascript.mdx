...
```js copy filename="main.js"
import FlagsJS from "@basestack/flags-js-sdk";
// or const FlagsJS = require("@basestack/flags-js-sdk").default;
...
To enhance your application's performance, our SDK includes caching features that enable you to use methods like **flags** and **flags**, providing instantaneous responses. To take advantage of this functionality, it's essential to initialize the request for the cache.
...
The SDK provides two alternatives for fetching a feature flag: the 'flag' and 'flagAsync' methods. Both serve the same purpose of retrieving a flag by its name. However, the 'flag' method uses the cached version, resulting in faster access, while 'flagAsync' fetches the latest version directly from the server.

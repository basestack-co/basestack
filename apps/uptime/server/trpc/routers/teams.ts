  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      return await ctx.prisma.$transaction(async (tx) => {
        ...
        const usersInForms = await tx.projectUsers.findMany({
          where: {
            userId: { in: userIds },
            project: {
              users: {
        ...
        if (usersInForms.length > 0) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: `Some team members are still assigned to projects: ${usersInForms
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await qstash.events.sendEmailEvent({
        ...
        props: {
          ...
          linkUrl: `${config.urls.getAppWithEnv(Product.UPTIME, AppMode as AppEnv)}/a/invite/${token}`,
        },
      });
